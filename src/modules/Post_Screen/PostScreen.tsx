...
import * as Location from 'expo-location'; // ✅ 新增导入定位库
...
export default function PostScreen({ navigation }: any) {
  ...
  const pickImage = async () => {
    ...
    if (status !== 'granted') {
      Alert.alert('Access required', 'Please allow access to the media library.');
      return;
    }
  ...
  const handlePost = async () => {
    if (!title || selectedImages.length === 0) {
      Alert.alert('Failed', 'Please add a title and image.');
      return;
    }
    ...
    try {
      ...
      if (status !== 'granted') {
        Alert.alert('Permission required', 'Location permission is needed to post.');
        return;
      }
      ...
      Alert.alert('Successful release', 'Your post has been saved to the database.');
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Text style={styles.sectionTitle}>Choose the tittle</Text>
